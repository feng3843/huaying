...
@interface MJWPlayerVC ()<SuperPlayerDelegate>
...
@property (nonatomic , strong) MJWPlayModel *selectedModel;
...
-(void)getVodDetailInfo{
    NSString *url = [NSString stringWithFormat:@"https://api.bjxkhc.com/index.php/app/ios/vod/show?device_token=E07054CF-1183-40C2-BB96-38C1DCA75936&id=%@&token=e413d222f0eda42b590e9598509b9522&uid=2277809",self.itemModel.movieId];
    [XPNetWorkTool requestWithType:HttpRequestTypeGet withHttpHeaderFieldDict:nil withUrlString:url withParaments:nil withSuccessBlock:^(NSDictionary *responseObject) {
        ...
        self.movieModel = [MJWMovieModel mj_objectWithKeyValues:responseObject[@"data"]];
        MJWzuModel *zu = [self.movieModel.zu firstObject];
        self.playLists = zu.ji;
        self.downloadLists = zu.ji;
        [self setUI];
...
-(void)setUI{
    ...
    NSString *mid = [NSString stringWithFormat:@"DD%@",self.itemModel.movieId];
    ...
    self.intoL.text = self.movieModel.text;
    self.nameL.text = self.movieModel.name;
    CGSize size = [self.movieModel.text boundingRectWithSize:CGSizeMake(screenW - 40, 100000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:12]} context:nil].size;
    ...
    for (int i = 0 ; i < self.self.playLists.count ;i++) {
        MJWPlayModel *model = self.playLists[i];
    ...
    x = 0;
       for (int i = 0 ; i < self. downloadLists.count ;i++) {
           MJWPlayModel *model = self.downloadLists[i];
...
-(void)btnClick:(UIButton *)btn{
    ...
    MJWPlayModel *model = self.playLists[btn.tag];
...
-(void)downbtnClick:(UIButton *)btn{
    ...
    MJWPlayModel *model = self.downloadLists[btn.tag];
    if (self.selectedModel != model) {
        self.selectedModel = model;
        //播放新的model
        NSLog(@"播放新的model");
...
-(void)PlayWithPlayModel:(MJWPlayModel *)model needSave:(BOOL)save{
    [self.playerView resetPlayer];
    if (save) {
        NSString *mid = [NSString stringWithFormat:@"DD%@",self.itemModel.movieId];
    ...
    self.playerModel.videoURL = model.purl;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self.urlL.text = model.purl;
    });
